...
const subjectConfig = {
  ...
  DAA: {
    ...
    description: 'Sorting, Searching, Greedy approach, Divide & Conquer, and Branch and Bound',
    icon: BookOpen,
    iconBg: 'bg-violet-50',
...
function SubjectSelection() {
  ...
  const getStatForSubject = (subjectKey) => {
    const stat = subjectStats.find(s => s._id === subjectKey);
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <nav className="sticky top-0 z-50 navbar-glass">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <NavLink to="/assignments" className="flex items-center gap-2 px-4 py-2.5 text-sm font-medium text-slate-600 hover:text-slate-900 hover:bg-slate-100 rounded-xl transition-all">
              <ClipboardList className="w-4 h-4" />
              <span className="hidden sm:inline">Assignments</span>
            </NavLink>
            {user?.role === 'admin' && (
              <NavLink to="/admin" className="flex items-center gap-2 px-4 py-2.5 text-sm font-medium text-amber-600 hover:text-amber-700 hover:bg-amber-50 rounded-xl transition-all">
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        <div className="text-center mb-12 animate-fade-in-up">
          ...
          <p className="text-lg text-slate-500">Select a subject to start solving problems</p>
        </div>
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between mb-6">
                    ...
                    <span className={`px-3 py-1.5 rounded-full ${config.badge} text-xs font-semibold`}>
